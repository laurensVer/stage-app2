...
server <- function(input, output, session) {
  ...
  tracking1 <- reactive({
    ...
    for (j in 1:length(Values_D3$area_D3)) {
      for (i in 1:length(Values_D2$area_D2)) {
        ar <- (Values_D3$area_D3[j] / Values_D2$area_D2[i]) / av_area_inc
        pc <- ((((CoMX_D3_px[j] - CoMX_D2_px[i])^2) + ((CoMY_D3_px[j] - CoMY_D2_px[i])^2))^(1/2))
        ar <- abs(1 - ar)
        area_rat <- c(area_rat, ar)
        pos_ch <- c(pos_ch, pc)
        score <- pc + 5 * ar
    ...
    scoretable <- matrix(scores, nrow = length(Values_D3$cellid_D3), ncol = length(Values_D2$cellid_D2), byrow = TRUE)
    bestmatch <- apply(scoretable, 1, which.min)
    print(str(bestmatch))
    for (i in 1:length(bestmatch)) {
  ...
  output$PC2 <- renderPlot({
    ...
    plot <- ggplot(test_result, aes(x = Xcoord, y = InvY, colour = as.factor(OrigID), alpha=Type_D2)) +
      geom_point(size = 0.1) +
      scale_color_manual(values = colsss)+
      scale_alpha_manual(values = alphasss)+
      theme(panel.background = element_rect(fill = "gray27")) +
  ...
  output$PC1.1 <- renderPlot({
    tracking <- tracking()
    inv_lowvalues <- tracking()$inv_lowvalues
